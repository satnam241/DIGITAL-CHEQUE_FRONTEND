...
const Plans: React.FC = () => {
  ...
  const fetchPlans = async () => {
    try {
      const res = await fetch(`${USER_AUTH}${ENDPOINTS.GET_PLAN}`);
      ...
      {plans.length === 0 ? (
        ...
        plans.map((plan) => (
          <div key={plan._id} className="plan-card">
            ...
            <ul className="plan-features">
              <li>
                <span className="checkmark">✓</span> {plan.cheques} cheques allowed
              </li>
              <li>
                <span className="checkmark">✓</span> Valid for {plan.durationDays} days
              </li>
            </ul>
